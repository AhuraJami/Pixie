...
public:
	 ...
	 * @tparam T (Automatically deduced) Type of input object that supposedly implements the Tick concept
	 * @param [in] x Input object that should implement the tick concept
	 */
	template<class T>
...
private:
	/**
	 * Base type erasure interface class that provides Tick concept
	 */
	struct Concept : public VirtualTick
	{
		/** Default virtual destructor */
		virtual ~Concept() = default;

		/**
		 * Interface of utility Copy method that is used in Copy constructor of parent (i.e. Tickable class)
		 */
		virtual std::unique_ptr<Concept> Copy() const = 0;
	};

	/**
	 * Derived model class that implements the Tick interface and calls the Tick function of the type erased
	 * object stored in its mebmer 'data'
	 * @tparam Type of input object that supposedly implements the Tick concept
	 */
	template<class T>
	struct Model final : public Concept
	{
		 ...
		 * @param x An object that supposedly implements the Tick concept
		 */
		Model(T x) : data(std::move(x))	{}

		/**
		 * Implementation of the utility copy interface method that helps with copying the member 'self' of the
		 * parent score
		 */
		...
		/** Type erased object that supposedly complies with Tick concept */
		T data;
	};

private:
	/** A unique pointer to type erased data that implements Tick concept */
